    ...
    try {
        ...
        if (Object.keys(data).length == 0) {
            return res.status(400).send({
                status: false,
                message:
                    "data should  be not Empty please enter some data to create cricketUser",
            });
        }
...
const getCric = async function (req, res) {
    try {
        ...
        let cricket = await cricketModel.find().populate('userId')
...
const getAllCric = async function (req, res) {
    try {
        ...
        const cricketData = await cricketModel.find(data).sort({ cricMatch: -1, hocRuns: -1, hocWins: -1 });
